...
```{r PCA}
RET.pca <- prcomp(RET_C[,c(1:10)], center = TRUE,scale. = TRUE)
summary(RET.pca)
str(RET.pca)
RET.pca$rotation
library(devtools)
#install_github("vqv/ggbiplot")
library(ggbiplot)
ggbiplot(RET.pca,labels=rownames(RET_C))
plot(RET.pca$x[,1], RET.pca$x[,2])
#sd
RET.pca_var <- RET.pca$sdev^2
RET.pca_var_per <- round(RET.pca_var/sum(RET.pca_var)*100,1)
barplot(RET.pca_var_per)
```
